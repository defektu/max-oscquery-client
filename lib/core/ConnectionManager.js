...
class ConnectionManager {
  ...
  async connect(url) {
    ...
    if (this.state.connected || this.wsClient) {
      // Disconnect but preserve URL for reconnection
      this.disconnect(true);
    }

    // Normalize and preserve URL immediately for reconnection
    const normalizedUrl = url ? url.replace(/\/$/, "") : this.state.baseUrl;
    if (!normalizedUrl) {
      throw new Error("No URL provided for connection");
    }

    // Set baseUrl before attempting connection so it's preserved on error
    this.state.baseUrl = normalizedUrl;

    // Ensure connection state is completely reset before new attempt
    ...
    if (this.wsClient) {
      try {
        ...
        this.callbacks.onLog("Error closing stale client: " + e.message);
      }
      this.wsClient = null;
    }
    ...
    try {
      this.callbacks.onLog(
        `Connecting to ${normalizedUrl} with timeout ${this.state.timeout}...`
      );
      // Fetch namespace
      ...
      this.state.connected = false;
      // Preserve baseUrl for potential auto-reconnect
      // baseUrl is already set above at line 77, so we keep it
      // Only clear other state, not baseUrl
      this.state.hostInfo = null;
      this.state.namespace = null;
      this.state.paramsDict = null;
      // Log baseUrl to verify it's preserved
      this.callbacks.onLog(
        `Connection failed, baseUrl preserved: ${this.state.baseUrl}`
      );
    ...
    return {
      ...
      onError: (error) => {
        if (this.state.connected) {
          this.state.connected = false;
          // Preserve baseUrl for potential auto-reconnect
          const savedBaseUrl = this.state.baseUrl;
          this.state.hostInfo = null;
          this.state.namespace = null;
          this.state.paramsDict = null;
          // Don't clear baseUrl here - let the disconnect handler decide
          this.callbacks.onDisconnected();
        }
        this.callbacks.onError(error);
      },
   ...
   * @param {boolean} preserveUrl - If true, preserves baseUrl for potential reconnection
   */
  disconnect(preserveUrl = false) {
    if (this.wsClient) {
      this.wsClient.close();
      this.wsClient = null;
    }
    ...
    const savedBaseUrl = this.state.baseUrl; // Save for potential reconnect
    if (!preserveUrl) {
